...
int nfVertex(FILE *);
NODE *crNode(int, char *);
GRAPH *crGraph(int);
GRAPH *addEdge(int, char *);
void printGraph(GRAPH *, int);
void enqueue(QUEUE **, int, char *);
QUEUE *dequeue(QUEUE **);
void BFS(GRAPH **, char *, int, int *, int *);
int findMaxDistance(int, int *);
int *fLBNumber(GRAPH *, int, int, int *);
void update(LL **, char *);
void insert(LL **, char *);
void sDistance(GRAPH *, char *, int, int *, int *, char *, LL **);
int main(int argc, char **argv)
{
	...
	BFS(&_graph, "Bacon, Kevin", n, visited, parent);
	int max = findMaxDistance(n, visited);
	int *frequency = (int *)calloc(max, sizeof(int));
	frequency = fLBNumber(_graph, max, n, visited);
	printf("\n");
	char dest_act[100];
	printf("----------------------------------------------------------------------------------------------------\n");
	...
	printf("----------------------------------------------------------------------------------------------------");
	printf("\n");
	while (strcmp(dest_act, "quit") != 0)
	{
		sDistance(_graph, "Bacon, Kevin", n, visited, parent, dest_act, &head);
		printf("\n");
		printf("----------------------------------------------------------------------------------------------------\n");
...
NODE *crNode(int vertex, char *movie_actor)
{
...
{
	GRAPH *_graph = crGraph(nVertices);
	FILE *fp = fopen(filename, "r");
	NODE *newNode;
	char *ptr1;
	char *ptr2;
...
void BFS(GRAPH **_graph, char actor[100], int n, int *visited, int *parent)
{
	for (int k = 0; k < n; k++)
	{
		visited[k] = -1;
...
int *fLBNumber(GRAPH *_graph, int max, int n, int *visited)
{
	int i, j;
	GRAPH *current = _graph;
...
void sDistance(GRAPH *_graph, char from[100], int n, int *visited, int *parent, char dest_act[100], LL **head)
{
	...
	while (current != NULL)
	{
		if (current != NULL && strcmp(current->name, dest_act) == 0)
		{
	...
	insert(head, dest_act);
	while (i < n && strcmp(storage[i], dest_act) != 0)
	{
		i++;
	}
	if (i == n)
	...
	else if (strcmp(dest_act, "Bacon, Kevin") == 0)
	{
